...
| credentials.`base_url` | 默认 **`https://api.x.ai/v1`** |
...
| `email` / JWT | `credentials.email`、`account.name` |
| `expired` / JWT `exp` | 账号 `expires_at`（unix）+ credentials `expires_at`（RFC3339） |
| `base_url` 含 `cli-chat-proxy.grok.com` | **改写为** `https://api.x.ai/v1` |
| — | `platform: "grok"` |
| — | `type: "oauth"` |
| client / scope | `credentials.client_id`、`scope` 等（可从 JWT 兜底） |

`extra` 中会带上 `import_source: "grok-register-cpa"`、`email_key` 等元数据，便于排查。

### 4.5 base_url 语义差异（重要）

| 格式 | 典型 base_url | 含义 |
| --- | --- | --- |
| CPA | `https://cli-chat-proxy.grok.com/v1` | CLIProxyAPI / Build 免费路径 |
| Sub2API | `https://api.x.ai/v1` | Sub2API Grok OAuth 导入约定 |

转换时由 `_normalize_base_url` 处理，避免把 cli-chat-proxy 原样导入 Sub2API 导致不可用。
...
| base_url 为何不同？ | CPA 用 cli-chat-proxy；Sub2API 导入用 api.x.ai/v1 |
