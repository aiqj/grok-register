...
def create_standalone_page(
    *,
    ...
    try:
        reg_file = PACKAGE_DIR / "core.py"
        if reg_file.is_file():
            reg_dir = str(_pkg_root)
            if reg_dir not in sys.path:
                sys.path.insert(0, reg_dir)
            try:
                from grok_register.core import create_browser_options  # type: ignore
    ...
    if headless:
        ...
        # create_browser_options() may have already enabled headless from config;
        # force visible for remint / --headed standalone mint.
        try:
            opts.headless(False)
        except Exception:
            pass
        for arg in ("--headless", "--headless=new", "--headless=chrome"):
            try:
                # DrissionPage may keep flags in different attrs; best-effort clear
                args = getattr(opts, "arguments", None) or getattr(opts, "_arguments", None)
...
def inject_cookies(page: Any, cookies: Any, log: LogFn | None = None) -> int:
    """Inject cookies into the mint browser.

    Device OAuth UI uses ``accounts.x.ai``. OIDC token endpoints on ``auth.x.ai``
    are called via HTTP (urllib), not by navigating the browser to the issuer root.
    Cookie domains (including auth hosts) are applied with CDP ``set.cookies``.
    """
    log = log or _noop_log
    ...
    try:
        ...
        log(f"cookie inject warm accounts.x.ai: {e}")
    ...
    # JS document.cookie for non-httpOnly SSO cookies (best effort; same host)
    try:
        js_items = [
            c
            for c in items
...
def is_cloudflare_block(url: str = "", text: str = "") -> bool:
    """Detect Cloudflare hard-block / challenge interstitial pages."""
    u = (url or "").lower()
    ...
    if any(m in t for m in markers):
        return True
    if "cdn-cgi/challenge" in u or "cf-browser-verification" in u:
        return True
    # Title-ish: "Attention Required" + cloudflare in body
    if "attention required" in t and "cloudflare" in t:
        return True
    if "you have been blocked" in t and ("x.ai" in t or "cloudflare" in t):
        return True
    return False
...
def _fill(page: Any, selector: str, value: str, log: LogFn, label: str = "field") -> bool:
    """Fill an input (email/password). Uses React-friendly value setter."""
    value = str(value or "")
    if not value:
        return False
    css = selector[4:] if selector.startswith("css:") else selector
    # Prefer JS native setter so React controlled inputs accept the value
    try:
        ok = page.run_js(
            """
...
try { n._valueTracker && n._valueTracker.setValue(''); } catch (e) {}
...
def approve_device_code(
    ...
    *,
    ...
    while time.time() < deadline:
        ...
        # Hard Cloudflare block — abort early (do not wait full oauth poll timeout)
        if is_cloudflare_block(url, text):
            snip = _norm(text)[:180]
            raise BrowserConfirmError(
                "cloudflare_blocked: mint browser cannot access x.ai "
                f"(url={url[:100]!r} visible={snip!r}). "
                "Prefer warm register browser (cpa_mint_prefer_warm_browser=true) "
                "or --headed / proxy."
            )

        # Done page — token poll runs in a side thread; avoid log spam
        if "device/done" in url or "设备已授权" in text or "device authorized" in text.lower():
            if not done_page_logged:
                log("device done page — waiting for token poll (HTTP)")
        ...
        # Device code entry (also recover from rate_limited query)
        if (
            page.ele("css:input[name='user_code']", timeout=0.3)
            and "consent" not in url
        ) or ("error=rate_limited" in url and "consent" not in url):
            phase = "device"
            if "error=rate_limited" in url:
                log("device page rate_limited — refill user_code and retry continue")
            if user_code:
                try:
                    uc = page.ele("css:input[name='user_code']", timeout=0.5)
                    if uc is not None:
                        cur = (uc.value or "") if uc else ""
                        if user_code.replace("-", "") not in str(cur).replace("-", ""):
                            try:
                                uc.clear()
                            except Exception:
                                pass
        ...
        if (
            "全部允许" in text
            or "隐私偏好" in text
            or "接受所有 Cookie" in text
            or "Accept all" in text
            or "Cookie 设置" in text
        ):
            if _click_exact(
                page,
                ["接受所有 Cookie", "全部允许", "Accept all", "Accept All Cookies", "全部拒绝"],
                log,
                real=False,
            ):
                _sleep(0.5)
        ...
        # Password login (SSO cookie missing/expired for this account)
        if page.ele("css:input[type='password']", timeout=0.3):
            phase = "password"
            # Surface auth errors instead of spinning forever
            low = text.lower()
            if any(
                ...
                log(f"login error banner: {_norm(text)[:120]}")
                if login_attempts >= 2:
                    raise BrowserConfirmError("password login failed (error banner)")
            if login_attempts >= 4:
                raise BrowserConfirmError(
                    "password login failed after retries; check account password/SSO"
                )
            login_attempts += 1
            log(f"login attempt {login_attempts} (email/password fallback)")
            # Email may already be filled; only re-fill if empty
            try:
                em = page.ele("css:input[type='email']", timeout=0.3)
                if em is not None:
                    cur = (em.value or em.attr("value") or "").strip()
                    if cur.lower() != email.lower():
                        _fill(page, "css:input[type='email']", email, log, "email")
            except Exception:
                _fill(page, "css:input[type='email']", email, log, "email")
            _wait_turnstile(page, log, 30)
            _fill(page, "css:input[type='password']", password, log, "password")
            _wait_turnstile(page, log, 15)
            ...
            # wait navigation away from password / sign-in
            left_login = False
            for _ in range(40):
                if stop_event is not None and stop_event.is_set():
                    return
                _sleep(0.5)
                u2 = _page_url(page)
                if "sign-in" not in u2 and not page.ele(
                    "css:input[type='password']", timeout=0.15
                ):
                    left_login = True
                    break
                if page.ele("css:input[type='password']", timeout=0.15) is None:
                    left_login = True
                    break
            if left_login:
                log("login appears successful — reopen device uri")
                try:
                    page.get(verification_uri_complete)
                    _sleep(2.0)
                    phase = "device"
                except Exception as e:
                    log(f"reopen device after login: {e}")
            continue
...
def mint_with_browser(
    *,
    ...
    # Pin "" for direct so urllib never picks up macOS system proxy (e.g. 127.0.0.1:1082).
    ...
    try:
        ...
        for attempt in range(1, 5):
            try:
                sess = request_device_code(proxy=resolved)
                last_err = None
                break
            except BaseException as e:  # noqa: BLE001
                last_err = e
                log(f"request_device_code attempt {attempt}/4 failed: {e}")
                err_l = str(e).lower()
                # On tunnel/proxy errors, force direct for remaining attempts
                if any(m in err_l for m in ("tunnel", "503", "proxy")):
                    resolved = ""
                    set_runtime_proxy("")
                    log("request_device_code: force proxy=direct after tunnel/proxy error")
                # OAuth rate limit: wait longer
                if "slow_down" in err_l or "429" in err_l or "rate" in err_l:
                    _sleep(4.0 + 3.0 * attempt)
                else:
                    _sleep(1.5 * attempt)
        if sess is None:
        ...
        # Warm register tab already has SSO — inject is best-effort enrichment.
        if cookies:
            n = inject_cookies(work_page, cookies, log=log)
            log(f"cookie inject count={n}")
            # Soft probe only. Do NOT hard-abort here: cold Chromium often sees a
            # temporary CF interstitial on accounts.x.ai home, while
            # /oauth2/device?user_code=… still works with SSO cookies (especially headed).
            try:
                work_page.get("about:blank")
                _sleep(0.2)
                # Prefer a light probe on the actual device entry (closer to real flow)
                probe_url = (
                    sess.verification_uri_complete
                    or sess.verification_uri
                    or "https://accounts.x.ai/oauth2/device"
                )
                work_page.get(probe_url)
                _sleep(1.2 if not headless else 0.8)
                url = _page_url(work_page)
                text = _visible_text(work_page)
                snip = _norm(text)[:120]
                log(f"post-inject session url={url[:120]} visible={snip}")
                hard = is_cloudflare_block(url, text) and (
                    "you have been blocked" in text.lower()
                    or "unable to access" in text.lower()
                )
                if hard:
                    if headless:
                        raise BrowserConfirmError(
                            "cloudflare_blocked after cookie inject (headless); "
                            "use --headed remint or warm register mint"
                        )
                    # Headed: wait briefly for challenge / human, then continue device flow
                    log(
                        "post-inject CF hard block — headed mode will continue to device "
                        "approval (do not abort early)"
                    )
                    for _ in range(8):
                        _sleep(1.0)
                        url = _page_url(work_page)
                        text = _visible_text(work_page)
                        if not (
                            is_cloudflare_block(url, text)
                            and (
                                "you have been blocked" in text.lower()
                                or "unable to access" in text.lower()
                            )
                        ):
                            log(f"post-inject CF cleared url={url[:100]}")
                            break
                    else:
                        log(
                            "post-inject still blocked — proceeding to approve_device_code anyway"
                        )
            except BrowserConfirmError:
                raise
            except Exception as e:
                log(f"post-inject check: {e}")

        stop_event = threading.Event()
        abort_box: dict[str, BaseException] = {}
        token_box: dict[str, Any] = {}
        err_box: dict[str, BaseException] = {}

        def _cancel() -> bool:
            if stop_event.is_set() and "token" not in token_box:
                return True
        ...
        def _poll() -> None:
            try:
                time.sleep(2)
                # Cap poll wall-clock: do not hang for full device_code expires_in (1800s)
                poll_wall = min(int(sess.expires_in), max(int(browser_timeout_sec) + 30, 120), 240)
                tr = poll_device_token(
                    sess.device_code,
                    interval=max(sess.interval, 5),
                    expires_in=poll_wall,
                    log=log,
                    cancel=_cancel,
                    proxy=resolved,  # "" = direct (do not pass None → system proxy)
        ...
        # On hard CF block, do not wait full poll timeout
        join_timeout = max(browser_timeout_sec, 60) + 30
        if browser_err is not None and "cloudflare_blocked" in str(browser_err).lower():
            join_timeout = 15
        ...
        if browser_err is not None and "cloudflare_blocked" in str(browser_err).lower():
            raise browser_err
        if "err" in err_box:
            # Prefer browser CF error over generic poll cancel/timeout
            if browser_err is not None and "cloudflare" in str(browser_err).lower():
                raise browser_err
            raise err_box["err"]
